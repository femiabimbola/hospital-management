...
const Footer = () => {
  return (
    <div className="mx-auto max-w-2xl sm:pt-14 px-4 sm:px-6 lg:max-w-7xl lg:px-8">
      ...
      {/* Upper side */}
      <div className="my-10 grid grid-cols-1 gap-y-12 sm:grid-cols-6 lg:grid-cols-12">
        {/* Column-1 */}
        <div className="sm:col-span-4 lg:col-span-5">
          ...
          <h3 className="font-medium text-[#363636] mt-5 mb-4 lg:mb-16">
            Get access to world class<br /> medical doctor anywhere you are.
          </h3>
        ...
        {products.map((product, index) => (
          ...
          <div key={product.id} className="sm:col-span-2 col-span-3 ">
            <p className="text-lg font-medium mb-5">{product.section}</p>
            <ul>
              {product.link.map((link: string, index: number) => (
                <li key={index} className="mb-2">
                  <Link
                    href={"/"}
                    className="text-base font-normal mb-6 space-links"
                  >
                    {link}
                  </Link>
                </li>
